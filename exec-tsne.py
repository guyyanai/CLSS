...
def load_embeddings_dict(dict_path: str) -> Tuple[List[str], List[int], np.ndarray]:
    ...
    clss_embeds_all_np = np.concatenate([clss_embeds_seq_np, clss_embeds_seq_stretch_np, clss_embeds_str_np])
...
def write_output(domain_ids: List[str], xfolds: List[int], tsne_results: np.ndarray, output_path: str):
    data_to_save = {

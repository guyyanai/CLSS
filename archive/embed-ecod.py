...
# Constants
structures_path = os.path.join("data", ecod_redundancy, "structures")
dataset_path = os.path.join("data", ecod_redundancy, "domains.csv")
checkpoint = os.path.join("checkpoints", "F100_b180_g8_h32_r10_AF2_1M.lckpt")
output_dir = os.path.join("outputs", ecod_redundancy)
cache_dir = "cache"


def get_cache_paths() -> Tuple[str, str]:
    loaded_domains_cache_path = os.path.join(
        cache_dir, f"{ecod_redundancy}_loaded_domains.pkl"
    )
...
def get_output_path() -> str:
    return os.path.join(output_dir, f"domain_embeddings_dict.pkl")


@cache_to_pickle(path_param_name="cache_path")
def load_domains(
    domain_ids: List[str], cache_path: str
) -> Tuple[List[str], List[str], List[torch.Tensor], List[str]]:
    sequences = []
    structures = []
    bad_ids = []
    ...
    for domain_id in tqdm(domain_ids, desc="Loading domains"):
        try:
            sequence, structure = load_domain_from_pdb(
                domain_id, structures_path, "ECOD-UID", False, ".pdbnum.pdb"
            )
    ...
    subsequences = [
        sample_subsequences(sequence, 1, 20, 60)[0] if len(sequence) > 20 else sequence
...
def compute_embeddings(
    ...
    esm2, esm2_tokenizer, esm2_projection_head, esm3_projection_head = load_clss(
        checkpoint
    )
    esm3 = load_esm3()
...
def write_embedding_dict(
    dataset: pd.DataFrame,
    sequence_embeddings: torch.Tensor,
    subsequences_embeddings: torch.Tensor,
    ...
    for index, domain in tqdm(dataset.iterrows(), total=dataset.shape[0]):
        try:
            ...
            print(
                f"An error occurred while setting dict for domain: {domain.domain_id}, index: {index}, exception: {e}"
            )
...
if __name__ == "__main__":
    ...
    sequences, subsequences, structures, bad_ids = load_domains(
        dataset.domain_id.tolist(), cache_path=loaded_domains_cache_path
    )

    dataset = dataset[~dataset.domain_id.isin(bad_ids)].reset_index()

    sequence_embeddings, subsequences_embeddings, structure_embeddings = (
        compute_embeddings(
            sequences, subsequences, structures, cache_path=embeddings_cache_path
        )
    )

    write_embedding_dict(
        dataset,
        sequence_embeddings,
